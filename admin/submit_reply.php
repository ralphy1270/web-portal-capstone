...
if(isset($_POST['submit_botton'])){
	...
	if(isset($_POST['suggestion_ta'])){
		...
		$errorMessage = $post->Downloadale_Post($DownloadName, $DownloadSize,$request_category, $suggestion_ta,$send_by,$downloadname_screen,$send_to);
		}
...
class Submit_Forms {
	...
	public function Downloadale_Post($file_name, $DownloadSize, $request_category, $suggestion_ta,$send_by,$downloadname_screen,$send_to){
		...
		if($check_empty != "") {
			...
			$query = mysqli_query($this->con, "INSERT INTO messages VALUES ('', '$request_category', '$suggestion_ta', '$DownloadName', '$DownloadSize' , 'no', 'no', '$date','','$send_to','$send_by','$downloadname_screen')");
			$message = "";
		}
		else {
...
<div class="submitforms_form">
	<form action="submit_reply.php?reply_to=<?php echo $send_to; ?>" method="POST" enctype="multipart/form-data">
		<div class="form-group">
		  ...
		  <br><br>
		    <select class="form-control" id="category" name="request_category">
		      <option disabled selected value> -- Select Letter of Submission -- </option>
		      <option>Submission of the Following: Request Letter & Proof of Event/Competition </option>
		      <option>Submission of the Following: Request Letter & Activity Proposal </option>
		      <option>Submission of the Following: Financial Assistance Letter </option>
		      <option disabled> -- Approval of Request Letter -- </option>
		      <option>Approved Request: Financial Assistance to Youth - Academic and Extracurricular Activites and Events</option>
		      <option>Approved Request: Sanguiniang Kabataan Request - Financial</option>
		      <option>Approved Request: Sanguiniang Kabataan Request - Sports Related</option>
		      <option>Approved Request: Sanguiniang Kabataan Request - Civic Activites</option>
		      <option>Approved Request: Sanguiniang Kabataan Request - Machineries and Equipment</option>
		      <option>Approved Request: Informative Request - Interviews</option>
		      <option>Approved Request: Informative Request - Research Studies</option>
		      <option>Approved Request: Event Partnership Request</option>
		    </select>
	  	</div>
	    <div class="form-group">
