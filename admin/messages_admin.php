...
if($message_id != "new"){
  ...
  $sent_by_admin = $row['send_to_admin'];
  $date = $row['date'];

  $query_user = mysqli_query($con, "SELECT * FROM users WHERE username='$sent_by_admin'");
    ...
    <div class="row message_tab_one">
      <div class="col-sm-4 for_inbox_outbox">
        <div class="inbox_message">
          <p>Sent Messages</p> 
          <a href="messages.php?u=new" class="message_tab_link">Inbox Messages</a>
      ...
      <div class="col-sm-8 for_message_box">
        <div class="message_output_box">
          ...
          <div class="convo_output_box">
            <?php 
              if($message_id != "new"){
                echo $message_inbox_obj->messages_output_inbox_admin($message_id);
              }
            ?>
          </div>
...
<script>
  ...
  $(document).ready(function() {
    ...
    $.ajax({
      url: "includes/handlers/ajax_load_message_outbox.php",
      type: "POST",
